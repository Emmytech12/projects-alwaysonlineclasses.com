			...
			case 'update_user_api':
				...
				if($check>0){
					...
					if(($fullname=='')||($email=='')||($mobile=='')){ ///start if 1
						...
						$usercheck=mysqli_query($conn,"SELECT email FROM user_tab WHERE email='$email' AND user_id!='$user_id' ");
						$useremail=mysqli_num_rows($usercheck);
							if ($useremail>0){ ///start if 5
								...
								mysqli_query($conn,"UPDATE user_tab SET fullname='$fullname',email='$email',mobile='$mobile' WHERE user_id='$user_id'") or die (mysqli_error($conn));
